...
| **`list_devices.py`** | `/api/device` | `GET` | `Authorization`, `pageSize` | `data` (list of devices), `code` |
| **`get_device_details.py`** | `/api/device/detail` | `GET` | `imei`, `Authorization` | `deviceBrief`, `account`, `vehicleBean` |
| **`fetch_mileage.py`** | `/api/device/miles` | `GET` | `imei`, `startTime`, `endTime`, `Authorization` | `miles`, `runTime` |
| **`device_commands.py`** | `/api/device/status` | `GET` | `imei`, `Authorization` | `acc` (Ignition), `oil` (Relay status) |
| **`check_and_immobilize_device.py`**| `/api/instruction/relay` | `POST` | `parameter`: "2" (Cut), `imeis`, `Authorization` | `code`, `result`, `uuid` |
...
### **Core Logic**
- **`main.py`**: Auth entry point. It checks `/api/login` first, but falls back to the official Open API method: calculating `md5(md5(appKey)+time)` and calling `POST /api/auth`.
...
### **Discovery & Data**
- **`list_devices.py`**: Primarily used to verify which IMEIs are assigned to the `testdemo` or `Akuka` account. It displays a count and a summary of every device found.
...
- **`get_auth_params.py`** (in `telematics/` root): Use this to generate raw JSON test bodies for Postman if needed.
