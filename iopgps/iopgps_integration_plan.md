...
Implement auth flow against https://open.iopgps.com/api/auth with signature md5(md5(appKey)+time), token cache in-instance only, and token refresh/retry on 10010/10011/10012.
...
Add seed entry in management/commands/seed_telematics_providers.py with name="IOPGPS" and app_key.sensitive=True.
...
Require app_id and app_key.
Set base_url = credentials.get("base_url", "https://open.iopgps.com").
...
Supported via POST /api/instruction/relay.
Use parameter: "2" for cut oil/electricity (immobilize).
Use parameter: "1" for restore oil/electricity.
Error Mapping
...
immobilize/restore triggers the relay instruction correctly.
